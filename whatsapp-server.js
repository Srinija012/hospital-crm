...
function formatJid(phone) {
  ...
  if (phone.endsWith('@s.whatsapp.net') || phone.endsWith('@lid')) return phone;
  const clean = phone.replace(/[^0-9]/g, '');
  if (!clean) return null;
  return `${clean}@s.whatsapp.net`;
}

// Asynchronous JID resolver with WhatsApp account existence & country code auto-detection
async function resolveJid(phone) {
  ...
  if (sock && connectionStatus === 'connected') {
    try {
      // 1. Try raw clean number
      const res1 = await sock.onWhatsApp(clean);
      if (res1 && res1.length > 0 && res1[0].exists) {
        return res1[0].jid;
      }

      // 2. If 10-digit number without country code, try prepending sender's country code (e.g. 91 for India)
      if (clean.length === 10 && userInfo && userInfo.id) {
        const countryCodeMatch = userInfo.id.match(/^(\d{1,3})/);
        const countryCode = countryCodeMatch ? countryCodeMatch[1] : '91';
        const candidate = countryCode + clean;
        const res2 = await sock.onWhatsApp(candidate);
        if (res2 && res2.length > 0 && res2[0].exists) {
          console.log(`[JID Resolver] Auto-resolved 10-digit phone ${clean} -> ${res2[0].jid}`);
          return res2[0].jid;
        }
      }
    } catch (err) {
      console.warn('[JID Resolver] Error resolving via onWhatsApp:', err.message);
    }
  }
...
async function startWhatsApp() {
  try {
    ...
    sock = makeWASocket({
      ...
      browser: ['Aegis Hospital CRM', 'Chrome', '120.0.0'],
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (qr) {
        ...
        try {
          ...
          console.log('[WhatsApp] Generated new QR code successfully.');
        } catch (err) {
          console.error('Failed to generate QR base64:', err);
        }
      } else if (connection === 'connecting' && !qrCodeData) {
      ...
      if (connection === 'close') {
        ...
        console.log(`Connection closed (code: ${statusCode}). Session expired/invalid: ${isLoggedOut}`);
        ...
        if (isLoggedOut) {
          console.log(`[WhatsApp] Session expired or invalid (code ${statusCode}). Purging session credentials to generate fresh QR code.`);
...
app.post('/api/connect', async (req, res) => {
  ...
  if (connectionStatus === 'disconnected' || req.body?.force) {
    purgeSessionData({ includeCreds: true });
  }
  startWhatsApp();
  ...
  try {
    ...
    for (let msg of list) {
      if (msg.status === 'pending' && new Date(msg.sendAt) <= now) {
        const jid = await resolveJid(msg.phone);
        if (jid) {
          try {
            let result;
            if (msg.attachment) {
