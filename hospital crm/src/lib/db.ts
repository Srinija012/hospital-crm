...
async function runMigration() {
  ...
  try {
    ...
    if (isComplete && isComplete.value === 'true') {
      // One-time purge: remove pre-seeded workflows & auto-reply rules from IndexedDB
      const purged = await db.appSettings.get('h_automation_purge_v1');
      if (!purged) {
        const OLD_WORKFLOW_IDS = ['wf-1', 'wf-2', 'wf-3', 'wf-4'];
        const OLD_AUTOREPLY_IDS = ['ar-1', 'ar-2', 'ar-3'];
        // Force-wipe stored workflows to empty array
        await db.appSettings.put({ key: 'h_workflows_up', value: [] });
        console.log('[Purge] Cleared all stored default workflows from IndexedDB.');
        // Force-wipe stored auto-reply rules to empty array
        await db.appSettings.put({ key: 'h_autoreplies_up', value: [] });
        console.log('[Purge] Cleared all stored default auto-reply rules from IndexedDB.');
        await db.appSettings.put({ key: 'h_automation_purge_v1', value: 'true' });
      }
      return;
    }
...
export async function dbSavePatient(patient: Omit<Patient, 'id'> & { id?: string }): Promise<Patient> {
  ...
  if (isNew) {
    // Workflow automation removed — no automated messages on patient registration.
    // Users can create their own automation rules via the WhatsApp Automation page.
  }
...
export async function dbMovePatientToTrash(patientId: string): Promise<void> {
  ...
  // Cancel any pending scheduled WhatsApp messages for this patient
  if (typeof window !== 'undefined' && patient.phone) {
...
export async function dbPermanentlyDeleteTrashedPatient(trashedId: string): Promise<void> {
  dbEnforceRole(['Clinic Admin', 'Super Admin']);
  const trashed = await db.trashedPatients.get(trashedId);
  // Cancel any pending scheduled messages for this patient before permanent deletion
  if (typeof window !== 'undefined' && trashed?.phone) {
    const cleanPhone = trashed.phone.replace(/[^0-9]/g, '');
    fetch(`${WHATSAPP_API_URL}/api/cancel-scheduled`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ phone: cleanPhone })
